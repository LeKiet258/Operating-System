...
void ExceptionHandler(ExceptionType which)
{
	...
	case SyscallException: //neu which la syscall thi...
		...
		case SC_Exec:
		{
			...
			char* fileName = User2System(address, maxBuffer);
			OpenFile *f = fileSystem->Open(fileName);
			
			if (f == NULL) {
				DEBUG('a', "\nERROR: Khong mo duoc tap tin nay!");
		...
		case SC_Join:
		{
			...
			int result = processTab->JoinUpdate(pid);
		...
		case SC_Exit:
		{
			...
			if(exitStatus == 0)
			{
				int result = processTab->ExitUpdate(exitStatus);
